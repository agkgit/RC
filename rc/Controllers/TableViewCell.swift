...
class TableViewCell: UITableViewCell {
	...
	internal func fill(cellData: RedConnectCallData) {
		...
		if let status = self.redConnectCallData.status {
			...
			switch status {
			case "normal":
				if self.redConnectCallData.free == false {
					self.statusImage.image = UIImage(named: "icon_call_success")
				} else {
					self.statusImage.image = UIImage(named: "icon_operator_done")
				}
